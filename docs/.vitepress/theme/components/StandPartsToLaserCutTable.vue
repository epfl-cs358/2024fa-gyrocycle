<template>
  <div class="warning custom-block" v-if="!showPreviews">
    ...
    <p>
      STL file previews are not available on smaller screens, please use a
      laptop or increase the size of your browser window.
    </p>
  </div>
  <table>
    <thead>
...
export default {
  ...
  created() {
    ...
    const files = import.meta.glob("/public/stl/to-be-laser-cut/stand/*.stl", {
      eager: true,
    });
    ...
    this.stlFiles = Object.keys(files).map((filePath) => {
      ...
      let productionPath = filePath.replace("/public", "/2024fa-gyrocycle");
      return {
        name: filename, // Extract file name
        url: productionPath, // Generate accessible URL
        dxfName: filename
          .replace("/stl/to-be-laser-cut/bicycle/", "/dxf/")
          .replace(".stl", ".dxf"),
        dxf: productionPath
          .replace("/stl/to-be-laser-cut/bicycle/", "/dxf/")
          .replace(".stl", ".dxf"),
